...
C:\\Program Files\\Epic Games\\UE\_5.7
...
All build output is streamed to the \*\*UE Reflect Watch\*\* pane in the Visual Studio Output window (\*\*View > Output\*\*, then select "UE Reflect Watch" in the dropdown). This shows the full UnrealBuildTool output including the actual compiler error, which is easier to read than the Error List.
